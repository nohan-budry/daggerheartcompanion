<script lang="ts">
    ...
    function convertMulti(input: string, type: string, tier: string): any {
        if (input.length == 0) {
            return '-';
        }
    ...
    function convertSingle(input: string, type: string, tier: string): any {
        if (input.length == 0) {
            return '-';
        }
        ...
        try {
            ...
            result.name = [];
            while (traits.filter(trait => peek(lines)!.includes(trait)).length == 0) {
                result.name.push(pop(lines)!);
            }
            let data = pop(lines)!.split(" ").reverse();
            while (!traits.includes(peek(data)!)) {
                result.name.push(pop(data)!);
            }
            result.name = result.name.join(" ");
            result.trait = pop(data)!;
            result.range = [];
            while (!peek(data)!.startsWith("d")) {
                result.range.push(pop(data)!);
            }
            result.range = result.range.join(" ");
            result.damage = pop(data)!;
            if (data.length === 0) {
                result.damageType = pop(lines)!;
                data = pop(lines)!.split(" ").reverse();
            } else {
                result.damageType = pop(data)!;
            }
            result.burden = pop(data)!;

            if (result.burden.endsWith('—')) {
                result.burden = result.burden.slice(0, result.burden.length - 1);
    ...
    let type = $state("Primary Weapon");
    let tier = $state("1");
    let input = $state("");
    let convertedInput = $derived(convertMulti(input, type, tier));
</script>
...
<div class="mb-4 flex gap-4 items-center">
    <label for="select" class="text-xl block">Type</label>
    <select bind:value={type} id="select" class="border p-2">
